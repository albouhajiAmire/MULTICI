...
const ChatBox = (props) => {
   ...
   const [joinUser, setJoinUser] = useState({ fullname: "", email: "" , phone : ""})
   const [joinError, setJoinError] = useState("")
   const [stuff, setStuff] = useState({ successMsg: "", errorMsg: "", loading: false })
   const [message, setMessage] = useState("")
   const [messages, setMessages] = useState([])
 
  const ChatId = (getCookie("chatId") && getCookie("chatId") !== "") ? getCookie("chatId") : false ;
  ...
  const handleJoinTyping = (e) => {
    ...
    if (e.target.name === "fullname") {
      ...
      else if (e.target.value.length < 2) setJoinError("please enter your correct name")
      else setJoinError("")
    }
    else if (e.target.name === "phone") {
      if (!e.target.value) setJoinError("phone is required")
      else if (e.target.value.length < 2) setJoinError("please enter your correct phone number")
      else setJoinError("")
    }
    else if (e.target.name === "email") {
      if (e.target.value.length < 2) setJoinError("email is required")
      else if (!ValidateEmail(e.target.value)) setJoinError("please enter your correct email")
      else setJoinError("")
    }
  ...
  const sendMessage = () => {
    if (message != "") {
      ...
      Create(data).then(({ data }) => {
        if (!data.err) {
          setMessages([...messages , { msg : message , chatId : ChatId , type : "msg" , from : "user"}])
          setStuff({ successMsg: "sent", errorMsg: "", loading: false })

          sendMsg(socket, { msg : message , chatId : ChatId , type : "msg" , from : "user"} , () => {
                setMessage("")
          })
  ...
  const sendFile = (e) => {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      CreateFile(formData).then(({ data }) => {
        ...
        if (!data.err) {
          ...
          Create({ fileId : data.msg , chatId : ChatId , type : "file" }).then(({ data }) => {

            if (!data.err) {

              setMessages([...messages , { fileId : data.msg , chatId : ChatId , type : "file" , from : "user" }])
              setStuff({ successMsg: "sent", errorMsg: "", loading: false })

              sendMsg(socket ,{ fileId : data.msg , chatId : ChatId , type : "file" , from : "user" }, () => {
                setMessage("")
              })
    ...
    const toggleChatBox = () => {
      document.querySelector('#toggle-chat-box > i').classList.toggle('fa-comment');
      document.querySelector('#toggle-chat-box > i').classList.toggle('fa-xmark');
      document.querySelector('#toggle-chat-box > i').classList.toggle('is-active');
    ...
    return (
      ...
      <div className="fabs">
       ...
       {stuff.loading && "loading..."}
       {stuff.errorMsg && stuff.errorMsg}
       {stuff.successMsg && stuff.successMsg}
            ...
            {!ChatId &&
              ...
              <span className="chat_msg_item chat_msg_item_admin">
                <div className="chat_avatar">
                  <img src="https://www.kindpng.com/picc/m/22-223941_transparent-avatar-png-male-avatar-icon-transparent-png.png" alt="" />
                </div>join to Send message to agent.

                <div>
                  <form className="message_form" onSubmit={(e) => handleSubmit(e)}>
                    <input placeholder="your fullname" name="fullname" onChange={(e) => handleJoinTyping(e)} />
                    <input placeholder="your phone number" name="phone" onChange={(e) => handleJoinTyping(e)} />
                    <input placeholder="Your email" name="email" onChange={(e) => handleJoinTyping(e)} />
        ...
        messages.map((msg , mi) => {
           ...
           return (
            ...
            ?
               <Fragment key={mi}>
               <span  className="chat_msg_item chat_msg_item_user">{msg.msg}</span>
               <span className="status">{moment(msg.createdAt).fromNow()}</span>

             </Fragment>
          
            :
             <Fragment key={mi}>
                   <div style={{ position: "relative" , right: "0px" , top: "0px" , width: "100%" ,minHeight : "100px"}}>

                     <span className="chat_msg_item chat_msg_item_user fileButton clickable" onClick={(e) => { toggleFileButton(e) }}>file...</span>
            ...
            :
             <Fragment key={mi}>
                   ...
                   <div style={{ position: "relative", right: "0px", top: "0px", width: "100%", minHeight: "10px" }}>
                     ...
                     <span style={{ position: "relative" }} className="chat_msg_item chat_msg_item_admin clickable fileButton" onClick={(e) => { toggleFileButton(e) }}>
                       <div className="chat_avatar">
                         <img src="https://www.kindpng.com/picc/m/22-223941_transparent-avatar-png-male-avatar-icon-transparent-png.png" alt="" />
                       </div >file...</span>
                   ...
                   <span className="status left">{moment(msg.createdAt).fromNow()}</span>
          ...
          {ChatId &&
            <div className="fab_field">
              ...
              <input type='file' id="fab_camera" onChange={(e) => { sendFile(e) }} style={{display : "none" }}/>
              <label htmlFor="fab_camera" id="fab_camera" className="fab">
                 <i className="fa-solid fa-camera" ></i>
              </label>


              <a id="fab_send"  disabled={stuff.loading} onClick={sendMessage} href="#test" className="fab"><i className="fa-solid fa-message"></i></a>

              <textarea onChange={(e) => { setMessage(e.target.value) }} id="chatSend" name="chat_message" placeholder="Send a message" className="chat_field chat_message" value={message}></textarea>
        ...
        <a id="toggle-chat-box" href="#test" onClick={toggleChatBox} className="fab"><i className="fa-solid fa-comment"></i></a>
